...
- **Junction conflict resolution** — a second pass in `TrafficSystem` detects cross-traffic at shared junctions: if two vehicles on perpendicular paths are both approaching the same intersection within 200px, the further one yields to a stop until the closer one clears. Same-direction traffic (parallel headings) is left to ACC; oncoming traffic is left to the oncoming filter. The junction yield can only further reduce speed, never increase it.
...
| `scripts/traffic_system.gd` | `TrafficSystem` | `RefCounted` | Adaptive Cruise Control + junction conflict resolution: each frame finds the nearest vehicle ahead in a ~30° forward cone (ACC) and resolves cross-traffic conflicts at shared junctions (further vehicle yields) |
